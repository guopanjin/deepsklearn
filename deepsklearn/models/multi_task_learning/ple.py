import torch
import torch.nn as nn
import torch.nn.functional as F
from deepsklearn.layers import MLPBlock
from deepsklearn.initializations import init_embedding
'''
input:
 {
    "f1":(batch_size,),
    "f2":(batch_size,),
    "f3":(batch_size,),
 },
 {
   "ctr_label",(batch_size,),
   "ctcvr_label",(batch_size,),
 }
'''
class TaskExpertBlock(nn.Module):
    def __init__(self,
                 task_input_dim:int,
                 num_experts:int=3,
                 expert_hidden_layers:tuple=(128,64,32)
                 ):
        super().__init__()
        self.task_input_dim=task_input_dim
        self.num_experts=num_experts
        self.expert_hidden_layers=expert_hidden_layers
        expert_list=[MLPBlock(input_dim=self.task_input_dim,hidden_layers=self.expert_hidden_layers) for _ in range(self.num_experts)]
        self.experts=nn.ModuleList(expert_list)

    def forward(self,x):
        experts_output=[expert(x) for expert in self.experts]
        experts_output=torch.stack(experts_output,dim=1)#(batch_size,num_experts,expert_output)
        return experts_output
class PLE(nn.Module):
    def __init__(self,
                 categorical_feature_columns:dict=None,
                 numerical_feature_columns:list=None,
                 num_experts:int=4,
                 embed_dim:int=32,
                 hidden_layer=(128,64,32)
                 ):
        super().__init__()
        self.categorical_feature_columns=categorical_feature_columns
        self.numerical_feature_columns=numerical_feature_columns
        self.embed_dim=embed_dim
        self.hidden_layer=hidden_layer
        self.num_experts=num_experts
        embedding_table_dict={feature_name:nn.Embedding(num_embeddings,self.embed_dim) for feature_name, num_embeddings in  self.categorical_feature_columns.items()}
        self.feature_embedding_table_dict=nn.ModuleDict(embedding_table_dict)
        mlp_input_dim=len(categorical_feature_columns)*self.embed_dim+len(numerical_feature_columns)

        self.shared_experts=TaskExpertBlock(task_input_dim=mlp_input_dim,
                                            num_experts=self.num_experts,
                                            expert_hidden_layers=hidden_layer)
        self.ctr_specific_experts=TaskExpertBlock(task_input_dim=mlp_input_dim,
                                                  num_experts=self.num_experts,
                                                  expert_hidden_layers=hidden_layer)
        self.ctcvr_specific_experts=TaskExpertBlock(task_input_dim=mlp_input_dim,
                                                    num_experts=self.num_experts,
                                                    expert_hidden_layers=hidden_layer)

        self.ctr_gate=nn.Linear(mlp_input_dim,self.num_experts*2)
        self.ctcvr_gate = nn.Linear(mlp_input_dim, self.num_experts * 2)

        self.ctr_out_linear=nn.Linear(hidden_layer[-1],1)
        self.ctcvr_out_linear = nn.Linear(hidden_layer[-1], 1)

        self.apply(lambda m:init_embedding(m))

    def forward(self,x,label_dict):
        categorical_feature_embeddings=[self.feature_embedding_table_dict[feature_name](x[feature_name]) for feature_name in self.categorical_feature_columns.keys()]
        categorical_feature_embeddings=torch.stack(categorical_feature_embeddings,dim=1) #(batch_size,category_feature_size,embed_dim)

        numerical_feature=[x[feature_name] for feature_name in self.numerical_feature_columns]
        numerical_feature=torch.stack(numerical_feature,dim=-1)#(batch_size,numerical_feature_size)
        categorical_feature=torch.reshape(categorical_feature_embeddings,(categorical_feature_embeddings.shape[0],-1))#(batch_size,category_feature_size*embed_dim)

        feature_input=torch.concat([categorical_feature,numerical_feature],dim=1)#(batch_size,numerical_feature_size+category_feature_size*embed_dim)

        shared_experts_output=self.shared_experts(feature_input)
        ctr_experts_output=self.ctr_specific_experts(feature_input)
        ctcvr_experts_output=self.ctcvr_specific_experts(feature_input)

        ctr_experts_fusion_output=torch.concat([shared_experts_output,ctr_experts_output],dim=1)
        ctcvr_experts_fusion_output=torch.concat([shared_experts_output,ctcvr_experts_output],dim=1)


        ctr_gate_weights=F.softmax(self.ctr_gate(feature_input),dim=1)
        ctr_gate_weights=torch.unsqueeze(ctr_gate_weights,dim=-1)
        ctr_gate_output=torch.sum(ctr_gate_weights*ctr_experts_fusion_output,dim=1)


        ctcvr_gate_weights = F.softmax(self.ctcvr_gate(feature_input), dim=1)
        ctcvr_gate_weights=torch.unsqueeze(ctcvr_gate_weights,dim=-1)
        cvr_gate_output=torch.sum(ctcvr_gate_weights * ctcvr_experts_fusion_output, dim=1)



        ctr_logits=self.ctr_out_linear(ctr_gate_output)
        ctr_logits=torch.squeeze(ctr_logits,dim=-1)


        ctcvr_logits = self.ctcvr_out_linear(cvr_gate_output)
        ctcvr_logits=torch.squeeze(ctcvr_logits,dim=-1)

        ctr_label = label_dict["click"].to(torch.float)
        ctcvr_label = label_dict["conversion"].to(torch.float)

        ctr_loss=F.binary_cross_entropy_with_logits(ctr_logits,ctr_label)

        ctcvr_loss=F.binary_cross_entropy_with_logits(ctcvr_logits,ctcvr_label)


        loss=ctr_loss+ctcvr_loss

        pctr=F.sigmoid(ctr_logits)
        pctcvr=F.sigmoid(ctcvr_logits)

        return {
            "loss": loss,
            "ctr_loss": ctr_loss,
            "ctr_label": ctr_label,
            "ctcvr_loss": ctcvr_loss,
            "ctcvr_label": ctcvr_label,
            "pctr": pctr,
            "pctcvr": pctcvr}