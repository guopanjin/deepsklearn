...
class MMOE(nn.Module):
    def __init__(self,
        ...
        expert_list=[ MLPBlock(input_dim=mlp_input_dim,hidden_layers=hidden_layer) for _ in range(self.num_experts)]
        self.experts=nn.ModuleList(expert_list)

        self.ctr_gate=nn.Linear(mlp_input_dim,self.num_experts)
        self.ctcvr_gate=nn.Linear(mlp_input_dim,self.num_experts)
    ...
    def forward(self,x,label_dict):
        ...
        experts_output=[expert(feature_input) for expert in self.experts]
        experts_output=torch.stack(experts_output,dim=1)#(batch_size,num_experts,expert_out_put)
        ctr_gate_output=self.ctr_gate(feature_input)
        ctr_gate_weigets=F.softmax(ctr_gate_output,dim=1)#(batch_size,num_experts)
        ctcvr_gate_output=self.ctcvr_gate(feature_input)
        ctcvr_gate_weights=F.softmax(ctcvr_gate_output,dim=1)

        ctr_weight_output=torch.sum(torch.unsqueeze(ctr_gate_weigets,dim=-1)*experts_output,dim=1)
        ctcvr_weight_output = torch.sum(torch.unsqueeze(ctcvr_gate_weights, dim=-1) * experts_output, dim=1)


        ctr_logits=self.ctr_out_linear(ctr_weight_output)
        ctr_logits=torch.squeeze(ctr_logits,dim=-1)


        ctcvr_logits = self.ctcvr_out_linear(ctcvr_weight_output)
